...
# Package-installation + default java; plasma = ^1 ^2 ^3 ^4 ^26 ^29 ^38 ^39 ^43 ^44 
  ...
  doas pacman -Syyu kmod libelf qt5-wayland qt6-wayland avogadrolibs sagemath seatd ttf-iosevka-nerd arm-none-eabi-gcc gdb openocd arm-none-eabi-gdb libopencm3 vulkan-intel lib32-vulkan-intel phonon-qt5-vlc dialog avahi-runit virtualbox-guest-utils virtualbox-guest-iso cups-filters qemu libvirt-runit virtualbox-host-dkms virtualbox nss-mdns intel-undervolt-runit cups-pdf thermald-runit tlp-runit cpupower-runit pahole cpio perl tar xz bitwarden-cli networkmanager-openvpn xmlto python-sphinx python-sphinx_rtd_theme graphviz imagemagick terminator noto-fonts-emoji wmctrl libnotify lm_sensors-runit pcmanfm-gtk3 gvfs man-db i3status-rust rust wallutils curl mako wget fzf python-pywal zsh-theme-powerlevel10k go make otf-font-awesome swayidle ttf-opensans gammastep foliate xorg-xlsclients zsh swappy zsh-autosuggestions glances zsh-syntax-highlighting zathura zathura-pdf-mupdf pipewire pipewire-alsa pipewire-pulse easyeffects sway arduino-avr-core openshot mousepad wine-staging kicad-library kicad-library-3d links gnome-mahjongg gnome-calculator cups-runit qutebrowser geogebra kalzium step gthumb unrar unzip texlive-most atom libreoffice-fresh nodejs rclone syncthing-runit wayland gimp ffmpegthumbs linux-hardened linux-hardened-headers alsa-utils alacritty rsync lutris xdg-desktop-portal-kde xdg-desktop-portal-wlr pipewire-media-session gnuplot python3 python-pip libva-intel-driver brightnessctl ld-lsb lsd imv freecad artools iso-profiles aisleriot bsd-games mpv iptables-nft nftables-runit ebtables dnsmasq brave-bin obs-studio firefox kicad libpipewire02 polkit-gnome moc steam mypaint grim android-tools figlet shellcheck jdk-openjdk meson gvfs-mtp tumbler xarchiver
...
# Installation of packages from AUR; cnrdrvcups-lb only because of Canon-printer
  ...
  yay -S stm32cubemx spicetify-cli avogadroapp bibata-rainbow-cursor-theme handlr-bin candy-icons-git tela-icon-theme sweet-gtk-theme-dark spotify bibata-rainbow-cursor-theme cnrdrvcups-lb otf-openmoji sunwait-git arduino-ide-beta-bin sway-launcher-desktop swaylock-fancy-git bastet freshfetch-bin cbonsai fuzzel nudoku clipman openrgb-bin osp-tracker macchina onlyoffice-bin standardnotes-bin revolt-desktop toilet
  yay -Scd
