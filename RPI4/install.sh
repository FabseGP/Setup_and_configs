...
# All packages to install
  ...
  if [[ "$docker" == "yes" ]]; then
    apk add neofetch git py3-pip nautilus haveged alpine-sdk kbd-bkeymaps curl wget i2c-tools lm_sensors perl lsblk e2fsprogs-extra networkmanager iptables docker docker-cli-compose tzdata mysql-client firefox mysql pipewire ttf-opensans pipewire-pulse libuser ksysguard libreoffice pavucontrol i3status fzf rclone syncthing rsync terminator fcron unrar unzip zsh zsh-autosuggestions zsh-syntax-highlighting neovim btrfs-progs mousepad ark vlc spectacle htop plasma nodejs-current npm lsof zathura zathura-pdf-poppler eudev sway swaylock swayidle mesa-dri-gallium xdg-desktop-portal-wlr xdg-desktop-portal-kde wl-clipboard gnome-calculator polkit-gnome pipewire-media-session scrot kdialog swaylockd
  elif [[ "$docker" == "no" ]]; then
    apk add neofetch git py3-pip nautilus haveged alpine-sdk kbd-bkeymaps curl wget lsblk i2c-tools lm_sensors perl e2fsprogs-extra networkmanager iptables podman podman-docker tzdata py3-podman podman-remote fuse-overlayfs shadow slirp4netns docker-cli-compose mysql-client firefox mysql pipewire ttf-opensans pipewire-pulse libuser ksysguard libreoffice pavucontrol i3status fzf rclone syncthing rsync terminator fcron unrar unzip zsh zsh-autosuggestions zsh-syntax-highlighting neovim btrfs-progs mousepad ark vlc spectacle htop plasma nodejs-current npm lsof zathura zathura-pdf-poppler eudev sway swaylock swayidle mesa-dri-gallium xdg-desktop-portal-wlr xdg-desktop-portal-kde wl-clipboard gnome-calculator polkit-gnome pipewire-media-session scrot kdialog swaylockd
  fi
